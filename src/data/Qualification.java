...
public class Qualification {
    ...
    private void generateEntities() {
        ...
        for (int i = 0; i < database.getSchoolTreeSize(); i++) {
            ...
            BinarySearchTree currentSchoolOOEntityTree = new BinarySearchTree();
            ...
            for (int j = 0; j < currentSchool.getStudentTreeSize(); j++) {
                ...
                if (currentStudent.getEvents()[0] == true) {
                    ...
                    Entity newEntity = new Entity(Event.Type.ORIGINAL_ORATORY, currentSchool, false, studentList, currentStudent.getCode(), (Integer) settingsArray[13], (Integer) settingsArray[18]);

                    for (int k = 0; k < judgeTree.size(); k++) {
                        Judge currentJudge = (Judge) judgeTree.getNodeData(k);

                        if (currentJudge.getSchoolName().equals(newEntity.getSchoolName())) {
                            newEntity.addBlockedJudge(currentJudge);
                        }
                    }

                    ooEntityTree.insert(newEntity, newEntity.getCode());
                    currentSchoolOOEntityTree.insert(newEntity, newEntity.getCode());
                }
            }

            for (int j = 0; j < currentSchoolOOEntityTree.size(); j++) {
                Entity currentEntity = (Entity) currentSchoolOOEntityTree.getNodeData(j);

                for (int k = 0; k < currentSchoolOOEntityTree.size(); k++) {
                    Entity otherEntity = (Entity) currentSchoolOOEntityTree.getNodeData(k);

                    if (otherEntity.getCode().equals(currentEntity.getCode())) {
                        continue;
                    } else {
                        currentEntity.addBlockedEntity(otherEntity); // set entities from the same school as blocked
                    }
                }
            }
        }
        ...
        for (int i = 0; i < database.getSchoolTreeSize(); i++) {
            ...
            BinarySearchTree currentSchoolOIEntityTree = new BinarySearchTree();
            ...
            for (int j = 0; j < currentSchool.getStudentTreeSize(); j++) {
                ...
                if (currentStudent.getEvents()[1] == true) {
                    Student[] studentList = {currentStudent};
                    Entity newEntity = new Entity(Event.Type.ORAL_INTERPRETATION, currentSchool, false, studentList, currentStudent.getCode(), (Integer) settingsArray[14], (Integer) settingsArray[18]);
                    ...
                    currentSchoolOIEntityTree.insert(newEntity, newEntity.getCode());
                }
            }

            for (int j = 0; j < currentSchoolOIEntityTree.size(); j++) {
                Entity currentEntity = (Entity) currentSchoolOIEntityTree.getNodeData(j);

                for (int k = 0; k < currentSchoolOIEntityTree.size(); k++) {
                    Entity otherEntity = (Entity) currentSchoolOIEntityTree.getNodeData(k);
        ...
        for (int i = 0; i < database.getSchoolTreeSize(); i++) {
            ...
            BinarySearchTree currentSchoolISEntityTree = new BinarySearchTree();
            ...
            for (int j = 0; j < currentSchool.getStudentTreeSize(); j++) {
                ...
                if (currentStudent.getEvents()[2] == true) {
                    Student[] studentList = {currentStudent};
                    Entity newEntity = new Entity(Event.Type.IMPROMPTU_SPEAKING, currentSchool, false, studentList, currentStudent.getCode(), (Integer) settingsArray[15], (Integer) settingsArray[18]);
                    ...
                    currentSchoolISEntityTree.insert(newEntity, newEntity.getCode());
                }
            }

            for (int j = 0; j < currentSchoolISEntityTree.size(); j++) {
                Entity currentEntity = (Entity) currentSchoolISEntityTree.getNodeData(j);

                for (int k = 0; k < currentSchoolISEntityTree.size(); k++) {
                    Entity otherEntity = (Entity) currentSchoolISEntityTree.getNodeData(k);
        ...
        for (int i = 0; i < database.getSchoolTreeSize(); i++) {
            ...
            BinarySearchTree currentSchoolDAEntityTree = new BinarySearchTree();
            ...
            for (int j = 0; j < currentSchool.getDAPairTreeSize(); j++) {
                ...
                Entity newEntity = new Entity(Event.Type.DUET_ACTING, currentSchool, false, studentList, currentStudentPair.getCode(), (Integer) settingsArray[16], (Integer) settingsArray[18]);
                ...
                currentSchoolDAEntityTree.insert(newEntity, newEntity.getCode());
            }

            for (int j = 0; j < currentSchoolDAEntityTree.size(); j++) {
                Entity currentEntity = (Entity) currentSchoolDAEntityTree.getNodeData(j);

                for (int k = 0; k < currentSchoolDAEntityTree.size(); k++) {
                    Entity otherEntity = (Entity) currentSchoolDAEntityTree.getNodeData(k);
        ...
        for (int i = 0; i < database.getSchoolTreeSize(); i++) {
            ...
            BinarySearchTree currentSchoolDebateEntityTree = new BinarySearchTree();

            for (int j = 0; j < currentSchool.getDAPairTreeSize(); j++) {
                DebateStudentPair currentStudentPair = (DebateStudentPair) currentSchool.getDebatePairTreeNodeData(j);

                Student[] studentList = currentStudentPair.getStudentArray();
                Entity newEntity = new Entity(Event.Type.DEBATE, currentSchool, false, studentList, currentStudentPair.getCode(), (Integer) settingsArray[17], (Integer) settingsArray[18]);
                ...
                daEntityTree.insert(newEntity, newEntity.getCode());
                currentSchoolDebateEntityTree.insert(newEntity, newEntity.getCode());
            }

            for (int j = 0; j < currentSchoolDebateEntityTree.size(); j++) {
                Entity currentEntity = (Entity) currentSchoolDebateEntityTree.getNodeData(j);

                for (int k = 0; k < currentSchoolDebateEntityTree.size(); k++) {
                    Entity otherEntity = (Entity) currentSchoolDebateEntityTree.getNodeData(k);
    ...
    public QualificationEvent[] getEventArray() {
        QualificationEvent [] returnEventArray = new QualificationEvent[eventArrayLength];
    ...
    public QualificationEvent getEventArrayElement(int index) throws ArrayIndexOutOfBoundsException {
    ...
    public boolean eventsQualificationScoresCollected() {
        for (int i = 0; i < eventArray.length; i++) {
            QualificationEvent currentEvent = eventArray[i];

            if (currentEvent == null) {
                return false;
            } else if (currentEvent.getNumQualificationScores() != currentEvent.getMaxNumQualificationScores()) {
                return false;
            }
        }
